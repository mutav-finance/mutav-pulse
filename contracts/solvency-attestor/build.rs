//! Embeds the verification_key.json (snarkjs) into the contract at build time, as
//! byte constants — no storage. Generates `vk.rs` in OUT_DIR; `lib.rs` includes it.
//!
//! Layout (same as Soroban's BN254 host):
//!   - G1: x || y, each Fq big-endian 32 bytes (64 bytes).
//!   - G2: x.c1 || x.c0 || y.c1 || y.c0 (Ethereum/Soroban order), each Fq BE 32 bytes (128).
//!
//! VK: by default reads `../../circuits/verification_key.json` (our single circuit);
//! overridable via env `VERIFIER_VK_JSON`.
//!
//! Lean version of Nethermind's mechanism (build.rs + circuit-keys): here we only
//! depend on serde_json + num-bigint, without dragging in ark-*.
